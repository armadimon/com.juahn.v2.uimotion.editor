...
- **곡선 프리셋 고르개** — 노드 인스펙터에서 `AnimationCurve` 필드 아래에 뜬다. 빈 곡선
  편집기 앞에서 "버튼 눌리는 느낌"을 손으로 그릴 수 있는 사람은 드물다. 검증된 모양을
  출발점으로 주고 거기서 손보게 한다. **못 박지 않는다** — 고른 뒤에는 그냥 곡선이다
- 곡선과 **시간을 함께** 넣는다. 모양과 시간은 같이 정해진 것이라, 팝 곡선을 0.27초로
  돌리면 저작한 모양이 나와도 감각이 다르다
- 고른 뒤 목록은 첫 항목으로 돌아간다. 지금 곡선이 어느 프리셋인지 표시하지 않는 것은
  의도다 — 손으로 고친 곡선을 프리셋 이름으로 계속 부르면 다시 열었을 때 그 이름을 보고
  원본 모양이라고 믿게 된다
- `Window > UI Motion > Generate Preset Graphs` — 완성된 연출 여섯. `ButtonBounce` ·
  `ButtonPressRelease` · `SlotPopReveal` · `SlotAccentSlam_Tier2` · `SlotAccentSlam_Tier3` ·
  `AttentionPulse`. 전부 `ScaleNode` 하나에 곡선을 꽂은 것이다
...
트리거의 진실이 두 곳(그래프의 선언 목록 · 표식 노드)에 있어 어긋날 수 있었고, "이 노드가
Start의 진입점"이라는 사실이 캔버스가 아니라 옆 패널에만 있었다. 이제 `TriggerNode`가
유일한 진실이고 선언 목록은 거기서 계산되는 파생값이다 — 슬롯 목록이 `[MotionSlot]`
필드에서 계산되는 것과 같다.

- `MotionTriggerNames` — 예약 이름 · 재발사 정책 · 위상 규약을 사람이 읽는 말로 옮긴다.
  노드 뷰 · 노드 인스펙터 · 트리거 패널 · 팔레트가 같은 문구를 쓰게 한다
- **노드 뷰가 트리거를 다르게 그린다.** 제목이 `Trigger: Start`이고 이름이 비면
  `Trigger: (이름 없음)`이다. 배경색이 셋으로 갈린다 — 예약 이름 · 직접 지은 이름 ·
  이름 없음(잘못). 제목 아래 부제에 재발사 정책이 뜬다. 트리거는 제목이 데이터에 딸려
  있으므로 인스펙터에서 이름을 고치면 캔버스가 따라온다(`MotionNodeView.RefreshLabels`)
- **트리거로는 간선을 이을 수 없다.** `AcceptsParent`를 두어 `GetCompatiblePorts`가
  양방향 모두 막는다. `AcceptsChildren`과 같은 이유로 포트 자체는 살려 둔다 —
  이미 그렇게 배선된 그래프를 열었을 때 그 간선이 보이고 지울 수 있어야 한다
- **노드 인스펙터가 트리거 이름을 드롭다운으로 받는다.** 예약 이름 셋을 먼저 보여 주고
  "직접 입력..."이 마지막이다. 오타 하나가 조용히 발사되지 않는 트리거를 만드는 것을
  막는다. 정책 드롭다운은 각 값이 무엇을 뜻하는지 툴팁과 한 줄 설명으로 함께 보여 주고,
  예약 이름을 골랐으면 그 위상 규약(`Start`가 끝나면 `Loop`가 자동 발사, `End`는 `Loop`를
  먼저 멈춤)을 알려 준다. 이름이 비었거나 겹치면 그 자리에서 잡는다
- **팔레트와 노드 검색 창에 `Trigger: Start`·`Loop`·`End`가 바로 만들어지는 항목으로
  들어갔다.** 노드를 놓고 이름을 다시 고르는 두 단계를 한 단계로 줄인다. 같은 이름이
  이미 있으면 만들지 않고 그 노드를 골라 보여 준다 — 겹치면 뒤엣것은 코어가 무시하므로
  만들어 봐야 발사되지 않는 노드가 하나 늘 뿐이다
- **트리거 패널이 편집기가 아니라 목차가 됐다.** 이름 · 정책 · 자식 수를 나열하고 누르면
  캔버스의 그 노드로 데려간다. 진입점을 고르는 UI는 없앴다 — 진입점이 노드 자신이라
  고를 것이 없다. 목록은 `graph.Triggers`가 아니라 노드를 직접 훑는다. 그 목록은 이름
  없는 것과 중복을 이미 걸러낸 뒤라 **고쳐야 할 것이 목록에서 사라지기** 때문이다
- `MotionGraphMigration` — `Window > UI Motion > Migrate Graphs`. 프로젝트의 모든 그래프에서
  옛 트리거 선언을 노드로 옮긴다. **되돌릴 수 없으므로 확인 대화상자를 띄우고**,
  결과를 개수만으로 알리지 않는다 — `MigrationReport`의 *이어지지 않은 트리거*는
  그래프를 문맥으로 붙여 경고로 남긴다. 그것을 조용히 넘기면 발사해도 아무 일이 없는데
  오류는 하나도 없는 상태가 된다. 트리거 패널도 옛 선언이 남은 그래프에 같은 길을 준다
- 그래프 에셋 인스펙터의 트리거 목록이 "진입 노드"가 아니라 **자식 수**를 보여 준다.
  진입점이 노드 자신이 되어 "없을" 수가 없어졌고, 대신 잘못될 수 있는 것은 그 아래가
  비는 것이다
- `MotionSampleGenerator`가 트리거 노드를 만들어 예시 노드에 잇는다. 예시가 트리거 노드
  자신인 경우에는 트리거를 더 만들지 않고 그 노드에 이름만 준다

실행 중에 나온 경고를 모으는 진단 창.

경고가 나는 자리가 여덟인데 전부 콘솔로만 나가고, `OnceLogger` 때문에 인스턴스당 한 번뿐이다.
콘솔을 지웠거나 다른 로그에 밀려 올라가면 그걸로 끝이다.

- `MotionDiagnostics` — `UnityMotionLog.Emitted`를 구독해 진단을 쌓는다. 같은
  `(수준, 본문, 오브젝트)`는 새 항목을 만들지 않고 횟수만 올린다. 상한 500개이고
  넘으면 오래된 것부터 버린다 — 방치형에서 몇 시간 돌리는 동안 메모리를 계속 먹으면 안 된다.
  플레이 시작 시 비우기는 기본으로 켜져 있고 `EditorPrefs`에 남는다.
  도메인 리로드에서 사라지는 것은 감수한다
- `MotionDiagnosticEntry` — **오브젝트 참조를 들지 않는다.** 진단은 그것을 낸 오브젝트보다
  오래 살아야 하는데(그것이 목록의 존재 이유다) 참조를 붙잡으면 파괴된 오브젝트를 놓아주지
  못한다. id와 이름·타입만 들고, 누를 때 그 id로 다시 찾는다. 사라졌으면 이름만 남는다
- `MotionObjectId` — 오브젝트를 참조 없이 가리키는 수와 되찾기. 6000.3이
  `GetInstanceID`·`InstanceIDToObject`를 폐기하고 `EntityId`로 옮긴 것을 여기 한 곳에 가둔다
- `MotionDiagnosticsWindow` (`Window > UI Motion > Diagnostics`) — 심각한 것부터,
  같은 수준이면 최근 것부터. 행마다 아이콘 · 본문 · 어디서 · 횟수. 행을 누르면 그
  오브젝트를 계층에서 고르고 핑한다. 수준별 필터, 비우기, 플레이 시작 시 비우기 토글.
  비어 있으면 무엇을 하면 되는지 알려 준다.
  **행 목록은 Layout 패스에서만 다시 만든다** — 재생 중에는 진단이 아무 때나 들어오므로
  매 이벤트 원본을 읽으면 Layout과 Repaint의 컨트롤 개수가 달라져 IMGUI가 예외를 던진다.
  선택 변경과 비우기도 같은 이유로 다음 패스의 맨 앞으로 미룬다

`MotionPlayer` 인스펙터의 재생 전 검사.

진단 창은 이미 일어난 일을 보여 준다. 그런데 그 경고들의 상당수는 **재생하기 전에 이미 알 수
있는 것들이다.**

- "그래프 검사"가 "재생 전 검사"가 됐다. 그래프 검사 결과에 바인딩 대조 결과를 더해
  하나의 요약으로 보여 주고, **각 항목을 펼쳐 볼 수 있다** — 예전에는 "오류 N · 경고 M"만
  보여 주고 무엇인지 알려면 그래프 창을 열어야 했다
- 비어 있는 슬롯을 한 줄 요약으로 올렸다. 슬롯 구역의 중복 경고는 없앴다 —
  그 자리에는 이미 빈 행의 배경색과 "(비어 있음)" 표시가 있다
- **타입이 맞지 않는 바인딩을 잡는다.** 판정은 `MotionSlots.Coerce(object, Type)`에
  맡긴다. 여기서 같은 규칙을 다시 구현하면 언젠가 갈라지고, 그때 인스펙터가 "괜찮다"고
  한 것이 런타임에 경고를 낸다
- **`Fade`가 쓸 수 없는 대상을 잡는다.** `FadeNode`의 슬롯 타입은 `Component`라 일반
  타입 검사로는 아무 컴포넌트나 통과하는데, 실제로 필요한 것은 `CanvasGroup`이나
  `Graphic` 중 하나다. 노드 타입을 특별 취급하는 유일한 자리이고 그 이유를 코드에 적었다
- 검사 목록은 그리는 도중이 아니라 `Classify`에서 한 번에 모은다. 줄 수가 패스 중간에
  바뀌면 IMGUI가 예외를 던지기 때문이다. 펼침 여부도 패스 진입 시점의 값으로 고정한다
